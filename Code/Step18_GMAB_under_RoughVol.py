...
under the Rough Volatility model for the dissertation:
...
def run_gmab_simulation_roughvol():
    """Run GMAB simulation under Rough Volatility model with dynamic hedging."""
    ...
    # Simulate stock price paths using Rough Volatility
    ...
    for path in range(n_paths):
        ...
        for i, t_idx in enumerate(rebalance_times[:-1]):  # Exclude maturity
            ...
            # Calculate GMAB value and delta (using GBM as proxy)
            value, delta = gmab_value_and_delta(
                ...
                model='bs' # Using GBM as proxy for delta hedging
            )
...
def save_results_and_plots_roughvol(results, unhedged_pnl, hedge_pnl, S, A):
    """Save simulation results, tables, and plots for Rough Volatility model."""
    ...
    results_df.to_csv(results_roughvol_file, index=False)
    print(f"Results saved to: {results_roughvol_file}")
...
#     with open(table_roughvol_file, 'w') as f:
#         f.write(latex_table)
#     print(f"LaTeX table saved to: {table_roughvol_file}")
    ...
    axes[0, 0].set_title('GMAB P&L Distributions (Rough Vol)')
    axes[0, 0].legend()
    ...
    axes[0, 1].set_title(f'Sample Stock Price Paths (Rough Vol, {sample_paths} of {n_paths:,})')
    ...
    axes[1, 1].set_title('Risk Metrics Comparison (Rough Vol)')
    ...
    plt.tight_layout()
    figure_file = f"{figures_dir}/gmab_analysis_roughvol.png"
    ...
    print()
    print("=" * 60)
    print("GMAB ROUGH VOL SIMULATION RESULTS")
    print("=" * 60)
...
if __name__ == "__main__":
    # Run GMAB simulation under Rough Volatility
    results, unhedged_pnl, hedge_pnl, S, A = run_gmab_simulation_roughvol()
    
    # Save results and create plots
    save_results_and_plots_roughvol(results, unhedged_pnl, hedge_pnl, S, A)
    
    print("\nGMAB Rough Vol analysis completed successfully!")
